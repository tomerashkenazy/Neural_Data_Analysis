...
def fit_model(dir_fun, ori_fun, name):
    ...
    for unit in range(nUnits):
        ...
        for direction in range(nDirections):
            firing_rates = [len(data_arr[unit, direction, rep][0]) / measure_time for rep in range(nRepetitions)] # firing rates per direction
        ...
        p0 = [np.max(mean_firing_rates), 1, stim_angles_rad[np.argmax(mean_firing_rates)]] # initial guess for parameters
...
# YOUR_CODE starts here
mean_FR_vector, std_FR_vector = fit_model(von_mises_direction, von_mises_orientation, 'Von Mises') # using fit_model function to get mean and std firing rates
